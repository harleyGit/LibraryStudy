...
weak_entry_for_referent(weak_table_t *weak_table, objc_object *referent)
{
    ...
    size_t begin = hash_pointer(referent) & weak_table->mask;// 这里通过 & weak_table->mask的位操作，来确保index不会越界(& weak_table->mask 位操作来确保index不会越界，这同我们平时用到的取余%操作是一样的功能。只不过这里改用了位操作，提升了效率)
    size_t index = begin;
    size_t hash_displacement = 0;
                      ...
                      id *referrer_id, bool crashIfDeallocating)
{
    ...
    // 正在析构的对象，不能够被弱引用
    // 判断referent_id是否能够被weak 引用。这里主要判断referent_id是否正在被析构以及referent_id是否支持weak引用。如果referent_id不能够被weak引用，则直接返回nil
    if (deallocating) {
