...
//  HttpDNS优化接入方案: https://juejin.cn/post/6844904144705339400
...
/// 拉取 DNS 配置, 拉取配置这里先直接从本地读取，实际项目的还是应该去请求后台接口获取数据
- (void)requestRemoteDNSList  {
    ...
    //NSJSONReadingAllowFragments选项，它表示解析器允许顶层对象不是数组或字典，可以是任何JSON中允许的数据类型，比如字符串、数字、布尔值等
...
// HTTPDNS转换: 转换ip(1.以 ip 替换接口域名; 2.添加域名到头部 host 字段; 3.将原 request 的 Cookie 设置到新 request)
- (NSURLRequest *)transfromHTTPDNSRequest:(NSURLRequest *)request {
    if ([self supportHTTPDNS:request]) {
        ...
        if (entity.ips && entity.ips.count > 0 && (ipAddress = entity.ips.firstObject))
        {
            ...
            //将URL字符串中第一个出现的 originalHost 替换为 ipAddress
            ...
            //在HTTP请求中，Host头字段用于指定目标服务器的主机名或IP地址。当你修改URL中的域名为IP地址时，如果不相应地修改HTTP请求头中的Host字段，服务器可能无法正确处理请求，因为服务器需要根据Host字段来确定请求的目标服务器。
